...
cols = ['symbol', 'lastTradePrice', 'volume', 'openInterest', 'CallPut', 'Expiry', 'strike', 'moneyness']
...
def omon(opt_df, rows = 0, sort_by='volume'):
    opt_df = opt_df[cols]
    opt_df = opt_df[(opt_df['moneyness'] <= 0.975) | (opt_df['moneyness'] >= 1.025)]
...
def vol_comparison(q, ticker,rows =0, date_lim=4):
    surface_bod, bod_omon = get_vol_surface_omon(q, ticker, date_lim,rows, bod=True)
...
def opt_monitor(q,freq,watchlist, rows,date_lim):
...
# opt_monitor(5,m.biotech, date_lim=4)